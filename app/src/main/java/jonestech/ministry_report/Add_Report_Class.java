...
public class Add_Report_Class extends Activity {
    ...
    public void save(View view){
        ...
        if(video.getText().toString().isEmpty()){vs = 0;}else{vs = Integer.parseInt(video.getText().toString());}
        ...
        if(hour.getText().toString().isEmpty() && mags.getText().toString().isEmpty() &&
                ...
                tract.getText().toString().isEmpty() && video.getText().toString().isEmpty() &&
                rvs.getText().toString().isEmpty() && study.getText().toString().isEmpty() &&
                pioncred.getText().toString().isEmpty()){
            Toast.makeText(getApplicationContext(), getString(R.string.no_report), Toast.LENGTH_LONG).show();
        }else{
            Integer id = (int) (getid);
            db.Insert(id, date, monthyear, year, h, m, br, bo, t, rv, s, co, pc, vs);
            c.requery();
